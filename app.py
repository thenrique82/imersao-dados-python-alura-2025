...
if not df_filtrado.empty:
    ...
    cargo_mais_frequente = df_filtrado['cargo'].mode().values[0]
else:
...
with col_graf1:
    if not df_filtrado.empty:
        ...
        grafico_cargos = px.bar(
            ...
            labels={'usd': 'Salário Médio', 'cargo': 'Cargo'}
        )
...
with col_graf2:
    if not df_filtrado.empty:
        grafico_hist = px.histogram(
            ...
            labels={'usd': 'Faixa salário (USD)', 'count': ''}
        )
        grafico_hist.update_layout(title_x=0.1)
        st.plotly_chart(grafico_hist, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para exibir no gráfico de distribuição de salários.")
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        grafico_paises = px.choropleth(
            ...
            title='Média de Salário por País (Data Scientist)',
            labels={'usd': 'Média de Salário (USD)'}
        )
        grafico_paises.update_layout(title_x=0.1)
        st.plotly_chart(grafico_paises, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para exibir o gráfico de Data Scientist por país.")
